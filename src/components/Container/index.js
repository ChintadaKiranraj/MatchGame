...
import SelectedTabsList from '../SelectedTabsList'
...
class Container extends Component {
  ...
  tick = () => {
    const {gameTimeLimit} = this.state
    if (gameTimeLimit >= 1) {
      this.setState(prevState => ({
        gameTimeLimit: prevState.gameTimeLimit - 1,
      }))
    }
  }
  ...
  compareThumbnailAndImageUrl = () => {
    const lengthImagesList = imagesList.length
    const {thumbnailId, matchId, Score} = this.state

    if (thumbnailId === matchId) {
      const randomIndex = Math.ceil(Math.random() * lengthImagesList - 1)
      const eachObj = imagesList[randomIndex]
      const {id} = eachObj
  ...
  getGameResults = () => {
    ...
    return (
      <div className="game-result-outer-container">
        <div className="game-result-container">
          ...
          <button type="button" className="button" onClick={this.onResetGame}>
            <img
  ...
  render() {
    ...
    const newArray = imagesList.filter(eacObj => eacObj.id === matchId)

    const {imageUrl} = newArray[0]

    const boolean = isGameOver === true || gameTimeLimit === 0
    return (
      <div className="bg-container">
        <HeaderNav gameTimeLimit={gameTimeLimit} Score={Score} />

        <div className="thumbnails-image-container">
          {boolean ? (
            this.getGameResults()
          ) : (
            <div className="game-body">
              {/* here the imageUrl means the big image */}
              {thumbnailClicked && this.compareThumbnailAndImageUrl()}

              {<img src={imageUrl} alt="match" style={{width: '80%'}} />}
              ...
              <ul className="category-image-container">
                {filteredImagesList.map(each => (
                  <SelectedTabsList
                    each={each}
                    key={each.id}
                    updateMatchedImageId={this.updateMatchedImageId}
                  />
                ))}
              </ul>
            </div>
